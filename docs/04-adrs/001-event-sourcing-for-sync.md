...
We will use **Event Sourcing** as the architectural pattern for Vault Mode's data synchronization.

**Key Components**:
1. **Events Table**: Immutable log of all highlight operations (create, update, delete)
...
❌ **Storage**: Events accumulate over time (mitigated by compression/archival)  
...
**Why not chosen**: Insufficient for robust cross-device sync with offline support
...
**Why not chosen**: Too complex for MVP, event sourcing provides sufficient benefits
