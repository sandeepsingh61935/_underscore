...
class ContentFingerprinter {
  ...
  private extractMainContent(doc: Document): string {
    ...
    body.querySelectorAll('header, footer, nav, aside, .ad, .advertisement').forEach(el => el.remove());
...
class XPathGenerator {
  generate(node: Node, offset: number): string {
    ...
    while (current && current !== document.body) {
      ...
      if (current instanceof Element && current.id && this.isStableId(current.id)) {
      ...
      if (current instanceof Element && this.hasUniqueClass(current)) {
        const className = Array.from(current.classList).find(c => this.isStableClass(c));
        if (className) {
...
interface RestorationStrategy {
  name: string;
  execute(data: HighlightData, doc: Document): Promise<RestorationResult | null>;
}

class XPathStrategy implements RestorationStrategy {
  name = 'xpath';
  
  async execute(data: HighlightData, doc: Document): Promise<RestorationResult | null> {
    try {
...
class PositionStrategy implements RestorationStrategy {
  ...
  private createRangeFromPosition(root: Node, start: number, end: number): Range | null {
...
class HighlightRestorer {
  ...
  async restore(data: HighlightData, doc: Document): Promise<RestorationResult> {
  ...
  private async logRestoration(highlightId: string, method: string, confidence: number): Promise<void> {
...
class ExportService {
  ...
  async createExportJob(
    ...
    const downloadUrl = `https://cdn.yourdomain.com/${key}?` +
...
class SyncClient {
  ...
  async connect(): Promise<void> {
    ...
    const wsUrl = `wss://api.yourdomain.com/sync?` +
...
**This document continues with Part 3 covering Testing Strategy, Deployment Architecture, and Monitoring...**
