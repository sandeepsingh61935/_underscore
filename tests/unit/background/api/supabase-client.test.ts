...
describe('SupabaseClient', () => {
  ...
  describe('Test 8: Network timeout (>5s) throws TimeoutError', () => {
    it('should abort request after 5 seconds', async () => {
      ...
      // Act & Assert
      await expect(client.createHighlight(highlightData)).rejects.toThrow(TimeoutError);
  ...
  describe("Test 12: Concurrent API calls don't corrupt shared state", () => {
    it('should handle 3 parallel createHighlight calls independently', async () => {
      ...
      expect(mockInsert).toHaveBeenCalledWith(expect.objectContaining({ id: 'h1' }));
      expect(mockInsert).toHaveBeenCalledWith(expect.objectContaining({ id: 'h2' }));
      expect(mockInsert).toHaveBeenCalledWith(expect.objectContaining({ id: 'h3' }));
