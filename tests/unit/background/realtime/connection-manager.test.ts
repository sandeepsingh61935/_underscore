...
describe('ConnectionManager', () => {
  ...
  describe('connect', () => {
    ...
    it('should trigger reconnect on failure', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.stringContaining('Scheduling reconnect attempt 1/5')
      );
  ...
  describe('disconnect', () => {
    it('should unsubscribe and set manually disconnected flag', async () => {
      ...
      const networkHandler = mockEventBus.on.mock.calls.find(
        (call: any[]) => call[0] === EventName.NETWORK_STATUS_CHANGED
      )[1];
  ...
  describe('Exponential Backoff', () => {
    ...
    it('should stop after max attempts', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Max reconnect attempts reached for realtime service'
      );
