...
describe('V2 ModeCard', () => {
  ...
  it('aria-pressed reflects isActive state', () => {
    ...
    expect(screen.getByText('Capture').closest('button')).toHaveAttribute(
      'aria-pressed',
      'true'
    );
  ...
  it('aria-disabled reflects isLocked state', () => {
    render(<ModeCard id="cloud" label="Memory" isLocked onClick={vi.fn()} />);
    expect(screen.getByText('Memory').closest('button')).toHaveAttribute(
      'aria-disabled',
      'true'
    );
  });
