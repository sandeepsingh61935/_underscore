...
import {
  ToastProvider,
  useToast,
} from '../../../src/ui-system/components/composed/Toast';
...
describe('V2 Toast', () => {
  ...
  it('does not use Style C, MD3, Tailwind palette, or shadcn utility classes', () => {
    ...
    expect(html).not.toMatch(
      /green-50|green-200|green-400|green-600|green-800|green-950/
    );
    expect(html).not.toMatch(/red-50|red-200|red-400|red-600|red-800|red-950/);
    expect(html).not.toMatch(
      /yellow-50|yellow-200|yellow-400|yellow-600|yellow-800|yellow-950/
    );
  ...
  it('limits visible toasts to maxToasts', () => {
    function Multi(): React.JSX.Element {
      const { addToast } = useToast();
      return (
        <button onClick={() => addToast({ title: 'X', variant: 'info' })}>push</button>
      );
    }
    render(
