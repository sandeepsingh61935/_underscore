...
describe('debug useProviderModels', () => {
  ...
  it('uses the mocked catalog', async () => {
    vi.mocked(getOpenRouterModels).mockResolvedValue([{ id: 'free-a', label: 'Free A', hint: 'free', requiresKey: false }]);
    const { result } = renderHook(() => useProviderModels('openrouter'));
    await waitFor(() => expect(result.current.models.length).toBeGreaterThan(0));
    expect(result.current.models).toEqual([{ id: 'free-a', label: 'Free A', hint: 'free', requiresKey: false }]);
  });
});
