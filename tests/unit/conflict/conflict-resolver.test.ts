...
import {
  SyncEventType,
  type SyncEvent,
} from '../../../src/background/events/interfaces/i-event-store';
...
describe('ConflictResolver', () => {
  ...
  describe('Strategy Validations - canResolve', () => {
    it('should allow applicable strategies for METADATA_CONFLICT', () => {
      ...
      expect(resolver.canResolve(conflict, ResolutionStrategy.LAST_WRITE_WINS)).toBe(
        true
      );
  ...
  describe('Resolution Execution', () => {
    ...
    it('should throw error for MANUAL resolution without UI', async () => {
      ...
      await expect(resolver.resolve(conflict, ResolutionStrategy.MANUAL)).rejects.toThrow(
    ...
    it('should throw error for incompatible strategy', async () => {
      ...
      await expect(resolver.resolve(conflict, ResolutionStrategy.MERGE)).rejects.toThrow(
        /cannot resolve conflict type/
      );
    });
    ...
    it('should resolve using KEEP_BOTH (keeps local, warns)', async () => {
      ...
      expect(spy).toHaveBeenCalledWith(
        expect.stringContaining('KEEP_BOTH'),
        expect.any(Object)
      );
    });
  });
  ...
  describe('Merge Strategy', () => {
    it('should merge payloads for MERGE strategy', async () => {
      ...
      (conflict.local[0] as any).payload = {
        id: 'h-1',
        tags: ['local'],
        color: 'yellow',
      };
