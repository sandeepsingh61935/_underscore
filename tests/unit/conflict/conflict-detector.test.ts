...
import type { HighlightUpdatedPayload } from '../../../src/background/events/event-types';
...
describe('ConflictDetector', () => {
  ...
  describe('Detection Logic', () => {
    ...
    it('should fail gracefully if event payload is missing ID', async () => {
      ...
      await expect(detector.detectConflicts([badEvent], [])).rejects.toThrow();
    });
    ...
    it('should NOT detect conflict if remote is after local', async () => {
      ...
      const remoteEvent = createEvent('evt-2', entityId, {
        'device-A': 1,
        'device-B': 1,
      });
    ...
    it('should handle detection errors gracefully', async () => {
      ...
      await expect(detector.detectConflicts([localEvent], [remoteEvent])).rejects.toThrow(
