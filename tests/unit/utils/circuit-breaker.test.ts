...
describe('CircuitBreaker', () => {
  ...
  describe('Edge Cases (Tricky)', () => {
    ...
    it('should handle async operation throwing errors', async () => {
      ...
      // Act & Assert
      await expect(circuitBreaker.execute(throwingOperation)).rejects.toThrow(
        'Async throw'
      );
  ...
  describe('Chrome Extension Reality (Real Scenarios)', () => {
    it('should handle storage quota exceeded error', async () => {
      ...
      const quotaExceededOp = vi
        .fn()
        .mockRejectedValue(new Error('QuotaExceededError: Quota exceeded'));
    ...
    it('should handle network offline (ERR_NETWORK_CHANGED)', async () => {
      ...
      const networkError = vi.fn().mockRejectedValue(new Error('ERR_NETWORK_CHANGED'));
