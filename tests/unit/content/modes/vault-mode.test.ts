...
describe('VaultMode - IndexedDB Persistence & 3-Tier Anchoring', () => {
    ...
    describe('Highlight Creation with IndexedDB', () => {
        ...
        it('should reject empty selection', async () => {
            ...
            // Act & Assert
            await expect(vaultMode.createHighlight(emptySelection, 'yellow'))
    ...
    describe('Highlight Removal from IndexedDB', () => {
        ...
        it('should handle removal of non-existent highlight gracefully', async () => {
            // Act & Assert
            await expect(vaultMode.removeHighlight('non-existent-id'))
    ...
    describe('Edge Cases', () => {
        it('should handle multiple highlights with different colors', async () => {
            // Arrange & Act
            const id1 = await vaultMode.createHighlight(createMockSelection('Yellow text'), 'yellow');
            const id2 = await vaultMode.createHighlight(createMockSelection('Blue text'), 'blue');
            const id3 = await vaultMode.createHighlight(createMockSelection('Green text'), 'green');
