...
describe('SprintMode - Event Sourcing & Persistence', () => {
  ...
  describe('Highlight Creation with Event Sourcing', () => {
    ...
    it('should reject empty selection', async () => {
      ...
      // Act & Assert
      await expect(sprintMode.createHighlight(emptySelection, 'yellow')).rejects.toThrow(
  ...
  describe('Highlight Removal with Event Sourcing', () => {
    ...
    it('should handle removal of non-existent highlight gracefully', async () => {
      // Act & Assert
      await expect(sprintMode.removeHighlight('non-existent-id')).resolves.not.toThrow();
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle multiple highlights with different colors', async () => {
      ...
      const id3 = await sprintMode.createHighlight(
    ...
    it('should handle storage errors gracefully', async () => {
      ...
      await expect(sprintMode.createHighlight(selection, 'yellow')).resolves.toBeTruthy();
    });
  });
});
