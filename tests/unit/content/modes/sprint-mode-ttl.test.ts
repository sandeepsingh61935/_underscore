...
describe('SprintMode - TTL Functionality', () => {
  ...
  describe('cleanExpiredHighlights()', () => {
    ...
    it('should remove multiple expired highlights', async () => {
      ...
      const expired1 = await sprintMode.createHighlight(
        createMockSelection('Expired 1'),
        'yellow'
      );
      const expired2 = await sprintMode.createHighlight(
        createMockSelection('Expired 2'),
        'blue'
      );
      const expired3 = await sprintMode.createHighlight(
        createMockSelection('Expired 3'),
        'green'
      );
      const fresh1 = await sprintMode.createHighlight(
        createMockSelection('Fresh 1'),
        'yellow'
      );
      const fresh2 = await sprintMode.createHighlight(
        createMockSelection('Fresh 2'),
        'blue'
      );
  ...
  describe('onActivate() - Automatic TTL Cleanup', () => {
    ...
    it('should log cleanup count on activation', async () => {
      ...
      const id1 = await sprintMode.createHighlight(
        createMockSelection('Expired 1'),
        'yellow'
      );
      const id2 = await sprintMode.createHighlight(
        createMockSelection('Expired 2'),
        'blue'
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle exactly at expiration boundary (edge timestamp)', async () => {
      // Arrange
      const id = await sprintMode.createHighlight(
        createMockSelection('Boundary test'),
        'yellow'
      );
      const now = Date.now();
