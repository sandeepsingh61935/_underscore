...
describe('VerificationView OTP hardening', () => {
    ...
    it('clears the OTP input and shows one honest message on verify failure', async () => {
        ...
        await waitFor(() => {
            expect(
                screen.getByText('That code is incorrect or has expired. Try again or resend a new code.')
            ).toBeInTheDocument();
        });
    ...
    it('locks the resend button using the server retryAfterMs on rate limit', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText('Too many attempts. Try again in 0:45.')).toBeInTheDocument();
        });
