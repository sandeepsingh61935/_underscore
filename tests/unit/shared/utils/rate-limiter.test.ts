...
describe('RateLimiter Unit Tests', () => {
  ...
  it('should allow attempts within the rate limit', () => {
    ...
    rateLimiter = new RateLimiter({ maxAttempts: 5, windowMs: 1000 }, mockLogger);
  ...
  it('should reset attempts after window expires', () => {
    // Arrange
    vi.useFakeTimers();
    rateLimiter = new RateLimiter({ maxAttempts: 3, windowMs: 1000 }, mockLogger);
  ...
  it('should log warning when rate limit is exceeded', () => {
    // Arrange
    rateLimiter = new RateLimiter({ maxAttempts: 2, windowMs: 1000 }, mockLogger);
  ...
  it('should handle rapid concurrent attempts correctly', () => {
    // Arrange
    rateLimiter = new RateLimiter({ maxAttempts: 10, windowMs: 1000 }, mockLogger);
  ...
  it('should block all attempts when maxAttempts is 0', () => {
    // Arrange
    rateLimiter = new RateLimiter({ maxAttempts: 0, windowMs: 1000 }, mockLogger);
  ...
  it('should allow only 1 attempt when maxAttempts is 1', () => {
    // Arrange
    rateLimiter = new RateLimiter({ maxAttempts: 1, windowMs: 1000 }, mockLogger);
