...
describe('mapAuthError', () => {
  it('always returns the same fixed message for sign-in, ignoring the underlying code (anti-enumeration)', () => {
    expect(mapAuthError('sign-in', { code: 'invalid_credentials' })).toBe(
      'Email or password is incorrect.'
    );
    expect(mapAuthError('sign-in', { code: 'user_not_found' })).toBe(
      'Email or password is incorrect.'
    );
    expect(mapAuthError('sign-in', { code: 'email_not_confirmed' })).toBe(
      'Email or password is incorrect.'
    );
  ...
  it('falls back to message-pattern matching when a Supabase error has no code (older GoTrue)', () => {
    expect(
      mapAuthError('verify-email-otp', { message: 'Token has expired or is invalid' })
    ).toBe('That code is incorrect or has expired. Try again or resend a new code.');
  });
  ...
  it('maps weak_password to a min-length message', () => {
    expect(mapAuthError('sign-up', { code: 'weak_password' })).toBe(
      'Password must be at least 8 characters.'
    );
    expect(mapAuthError('update-password', { code: 'weak_password' })).toBe(
      'Password must be at least 8 characters.'
    );
  });
  ...
  it('falls back to a sensible default for unrecognized codes per context', () => {
    ...
    expect(mapAuthError('update-password', null)).toBe(
  ...
  it('never distinguishes oauth failures from sign-in copy', () => {
    expect(
      mapAuthError('oauth', {
        message: 'Missing authentication tokens in OAuth redirect',
      })
...
describe('isExistingAccountSignup', () => {
  ...
  it('is false for a normal new signup with populated identities', () => {
    expect(
      isExistingAccountSignup({ identities: [{ id: 'x' }] as unknown[] }, null)
    ).toBe(false);
  });

  it('is false when a session is present (e.g. auto-confirm enabled)', () => {
    expect(isExistingAccountSignup({ identities: [] }, { access_token: 't' })).toBe(
      false
    );
  });
