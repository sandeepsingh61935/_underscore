...
describe('Sprint Mode - Error Recovery', () => {
  ...
  it('should recover from storage errors', async () => {
    ...
    mockStorage.saveEvent = vi.fn().mockRejectedValue(new Error('Storage write failed'));
  ...
  it('should handle EventBus errors gracefully', async () => {
    ...
    await expect(sprintMode.createHighlight(mockSelection, 'yellow')).rejects.toThrow(
      'EventBus error'
    );
