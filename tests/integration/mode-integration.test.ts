...
describe('Mode Integration Tests', () => {
  ...
  describe('Data Isolation', () => {
    it('should maintain separate repositories for each mode', async () => {
      ...
      const walkId = await walkMode.createHighlight(
        createMockSelection('Walk'),
        'yellow'
      );
      const sprintId = await sprintMode.createHighlight(
        createMockSelection('Sprint'),
        'blue'
      );
      const vaultId = await vaultMode.createHighlight(
        createMockSelection('Vault'),
        'green'
      );
