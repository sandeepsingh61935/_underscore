...
const createMockNetworkDetector = (
  initialOnline = false
): INetworkDetector & { setOnline: (online: boolean) => void } => {
...
describe('OfflineQueue Integration Tests', () => {
  ...
  describe('Offline Queueing', () => {
    ...
    it('should emit OFFLINE_EVENT_QUEUED event', async () => {
      ...
      eventBus.on('OFFLINE_EVENT_QUEUED', (data) => {
  ...
  describe('Auto-Sync on Reconnection', () => {
    it('should sync events when network comes back online', async () => {
      ...
      eventBus.on('OFFLINE_EVENT_READY', (data) => {
        syncEvents.push(data);
      });
      eventBus.on('OFFLINE_SYNC_STARTED', (data) => {
        syncEvents.push(data);
      });
      eventBus.on('OFFLINE_SYNC_COMPLETE', (data) => {
        syncEvents.push(data);
      });
  ...
  describe('Edge Cases', () => {
    ...
    it('should emit OFFLINE_MODE_ENABLED when going offline', async () => {
      ...
      eventBus.on('OFFLINE_MODE_ENABLED', (data) => {
