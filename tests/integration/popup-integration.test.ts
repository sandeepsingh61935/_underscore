...
describe('Popup Integration - Realistic Scenarios', () => {
  ...
  describe('Network Glitch Recovery', () => {
    ...
    it('should fail after exhausting retries', async () => {
      ...
      (chrome.runtime.sendMessage as any).mockImplementation(
        (_msg: any, callback: any) => {
          (chrome.runtime as any).lastError = { message: 'Persistent network error' };
          callback(null);
        }
      );
  ...
  describe('Circuit Breaker Integration', () => {
    it('should open circuit after repeated failures', async () => {
      ...
      (chrome.runtime.sendMessage as any).mockImplementation(
        (_msg: any, callback: any) => {
          (chrome.runtime as any).lastError = { message: 'Service unavailable' };
          callback(null);
        }
      );
      ...
      const failures = [];
      for (let i = 0; i < 5; i++) {
        failures.push(stateManager.initialize(123).catch(() => 'failed'));
      }
    ...
    it('should transition to HALF_OPEN and recover', async () => {
      ...
      (chrome.runtime.sendMessage as any).mockImplementation(
        (_msg: any, callback: any) => {
          if (shouldFail) {
            (chrome.runtime as any).lastError = { message: 'Service down' };
            callback(null);
          } else {
            (chrome.runtime as any).lastError = null;
            callback({
      ...
      (chrome.runtime.sendMessage as any).mockImplementation(
        (_msg: any, callback: any) => {
          ...
          if (callCount <= 2) {
            ...
            (chrome.runtime as any).lastError = { message: 'Switch failed' };
            callback(null);
          }
        }
      );
    ...
    it('should work with full decorator chain (Circuit + Retry + Chrome)', async () => {
      ...
      (chrome.runtime.sendMessage as any).mockImplementation(
        (_msg: any, callback: any) => {
          ...
          if (callCount % 2 === 1) {
            (chrome.runtime as any).lastError = { message: 'Flaky network' };
            callback(null);
          } else {
            (chrome.runtime as any).lastError = null;
            callback({
