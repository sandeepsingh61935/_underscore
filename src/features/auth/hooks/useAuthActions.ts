...
export function useAuthActions(): UseAuthActionsResult {
    const verifyEmailOtpAction = useIpcAction<{ email: string; token: string }, AuthResponse>(VERIFY_EMAIL_OTP);
    const resendEmailOtpAction = useIpcAction<{ email: string }, AuthResponse>(RESEND_EMAIL_OTP);
    const requestPasswordResetAction = useIpcAction<{ email: string }, Record<string, never>>(REQUEST_PASSWORD_RESET);
    const verifyRecoveryOtpAction = useIpcAction<{ email: string; token: string }, AuthResponse>(VERIFY_RECOVERY_OTP);
    const updatePasswordAction = useIpcAction<{ password: string }, AuthResponse>(UPDATE_PASSWORD);

    const verifyEmailOtp = async (email: string, token: string): Promise<AuthActionResult> => {
        const result = await verifyEmailOtpAction({ email, token });
        return result.success ? { success: true } : { success: false, error: result.error, code: result.code, retryAfterMs: result.retryAfterMs };
    };
    ...
    const verifyRecoveryOtp = async (email: string, token: string): Promise<AuthActionResult> => {
    ...
    return { verifyEmailOtp, resendEmailOtp, requestPasswordReset, verifyRecoveryOtp, updatePassword };
}
