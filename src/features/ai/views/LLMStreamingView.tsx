...
export function LLMStreamingView({
  ctx,
  onClose,
}: LLMStreamingViewProps): React.ReactElement {
  ...
  return (
    ...
    >
      <h2 className="u-serif" style={{ margin: 0 }}>
        Summary
      </h2>
      <div
        role="status"
      ...
      {error && (
        <div style={{ color: 'var(--ink)', font: 'var(--sans)' }}>Error: {error}</div>
      )}
      <div style={{ display: 'flex', gap: 'var(--step-1)' }}>
        <button type="button" onClick={abort} disabled={status !== 'streaming'}>
          Stop
        </button>
