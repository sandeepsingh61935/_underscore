...
export class ConnectionManager {
  ...
  public async connect(userId: string): Promise<void> {
    ...
    try {
      ...
      void this.cloudHydrationService.hydrate().catch((error) => {
        this.logger.error(
          'Incremental hydration after realtime connect failed',
          error as Error
        );
      });
    } catch (error) {
  ...
  private async handleReconnect(): Promise<void> {
    ...
    this.logger.info(
      `Scheduling reconnect attempt ${this.reconnectAttempts + 1}/${this.MAX_ATTEMPTS} in ${delay}ms`
    );
  ...
  private listenToNetworkChanges(): void {
    ...
    if (typeof window !== 'undefined') {
      window.addEventListener('online', () => {
        this.logger.info(
          'Network online (window event), attempting to reconnect realtime'
        );
