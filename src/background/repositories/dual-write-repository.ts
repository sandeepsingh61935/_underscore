...
export class DualWriteRepository implements IHighlightRepository {
  ...
  async add(highlight: HighlightDataV2, options?: RepositoryOptions): Promise<void> {
    ...
    if (options?.skipSync) {
      this.logger.info('[DualWrite] [SKIP] Skipping cloud sync (requested)', {
        id: highlight.id,
      });
      return;
    }
    ...
    if (currentUser) {
      ...
      this.writeToCloudAsync(
        () => this.cloudRepo.add(dataWithUser),
        'add',
        highlight.id,
        dataWithUser
      );
  ...
  async update(
    ...
    this.writeToCloudAsync(
      () => this.cloudRepo.update(id, updates),
      'update',
      id,
      updates
    );
  ...
  async findByUrl(url: string): Promise<HighlightDataV2[]> {
    ...
    if (this.authManager.currentUser) {
      this.logger.info('[DualWrite] User authenticated, fetching from cloud...', {
        userId: this.authManager.currentUser.id,
      });
    ...
    if (cloudOnlyIds.length > 0) {
      this.logger.info(
        '[DualWrite] [REPAIR] Read Repair: Backfilling cloud highlights to local',
        {
      ...
      this.backfillToLocal(
        cloudItems.filter((item) => cloudOnlyIds.includes(item.id))
      ).catch((error) => {
  ...
  private writeToCloudAsync(
    ...
    operation()
      ...
      .catch((error) => {
        // Log error
        this.logger.error(
          '[DualWrite] Cloud write failed (local still succeeded)',
          error,
          {
            operation: operationName,
            id: identifier,
