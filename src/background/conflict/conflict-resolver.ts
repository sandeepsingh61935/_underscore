...
import { ResolutionStrategyError, UnresolvableConflictError } from './conflict-errors';
...
export class ConflictResolver implements IConflictResolver {
  ...
  async resolve(conflict: Conflict, strategy: ResolutionStrategy): Promise<SyncEvent> {
    ...
    try {
      ...
      const timestamp =
        strategy === ResolutionStrategy.MERGE ? Date.now() : resolvedEvent.timestamp;
  ...
  private async resolveKeepBoth(conflict: Conflict): Promise<SyncEvent> {
    ...
    this.logger.warn(
      'KEEP_BOTH strategy in Resolver currently keeps Local and drops Remote (cloning requires state access)',
      {
        conflictId: conflict.id,
      }
    );
    ...
    if (
      ...
      if (localPayload.metadata && remotePayload.metadata) {
        ...
        if (localPayload.metadata.tags && remotePayload.metadata.tags) {
          mergedPayload.metadata.tags = Array.from(
            new Set([...remotePayload.metadata.tags, ...localPayload.metadata.tags])
          );
        }
      }
