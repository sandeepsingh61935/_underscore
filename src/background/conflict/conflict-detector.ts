...
export class ConflictDetector implements IConflictDetector {
   ...
   */
  async detectConflicts(local: SyncEvent[], remote: SyncEvent[]): Promise<Conflict[]> {
    ...
    try {
      ...
      for (const [entityId, localEvents] of localByEntity) {
        ...
        const conflict = await this.checkConflict(entityId, localEvents, remoteEvents);
        if (conflict) {
      ...
      throw new ConflictDetectionError('Failed to detect conflicts', {
  ...
  private async checkConflict(
    ...
    const conflictType = this.determineConflictType(latestLocal, latestRemote);
   ...
   * @private
   */
  private determineConflictType(local: SyncEvent, remote: SyncEvent): ConflictType {
    // Delete conflict: one side deleted, other modified
    if (
      local.type === SyncEventType.HIGHLIGHT_DELETED ||
      remote.type === SyncEventType.HIGHLIGHT_DELETED
    ) {
    ...
    if (
      local.type === SyncEventType.HIGHLIGHT_UPDATED &&
      remote.type === SyncEventType.HIGHLIGHT_UPDATED
    ) {
  ...
  private extractEntityId(event: SyncEvent): string {
    const payload = event.payload as
      HighlightCreatedPayload | HighlightUpdatedPayload | { id: string };
    return payload.id;
  }
  ...
  private updateDetectionMetrics(latency: number): void {
    ...
    this.metrics.averageDetectionLatency = (currentAvg * total + latency) / (total + 1);
