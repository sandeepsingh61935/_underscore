...
function makeRegistry(providers: Map<string, ILLMService>) {
  return {
    ...
    list: () =>
      Array.from(providers.entries()).map(([name]) => ({
        name: name as any,
        configured: true,
      })),
    setConfigured: vi.fn(),
...
describe('registerAiHandlers', () => {
  it('IPC_AI_LIST_PROVIDERS returns registered providers', async () => {
    ...
    const registry = makeRegistry(
      new Map([['anthropic', { providerName: 'anthropic' } as unknown as ILLMService]])
    );
    registerAiHandlers({
      bus: bus as any,
      registry: registry as any,
      keyStore: makeKeyStore() as any,
      pageContentCache: makePageContentCache() as any,
    });
    const handler = bus.handlers.get('IPC_AI_LIST_PROVIDERS')!;
    const result = await handler({});
    expect(result).toEqual({
      success: true,
      data: [{ name: 'anthropic', configured: true }],
    });
  });
  ...
  it('IPC_AI_HEALTH_CHECK returns provider health', async () => {
    ...
    expect(result).toEqual({
      success: true,
      data: { ok: true, model: 'claude-sonnet-4-6' },
    });
  });
  ...
  it('IPC_AI_GET_API_KEY_STATUS returns configured model', async () => {
    const bus = makeMessageBus();
    const keyStore = makeKeyStore();
    (keyStore.getModel as ReturnType<typeof vi.fn>).mockResolvedValue(
  ...
  it('IPC_AI_GET_PAGE_CONTEXT builds marked context from cache', async () => {
    ...
    const result = (await handler({
      ...
      data: {
        ...
        highlightExcerpts: Array<{ excerpt: string }>;
      };
    };
    ...
    expect(result.data.highlightExcerpts[0]?.excerpt).toContain(
