    ...
    try {
      ...
      while (cursor) {
        ...
        try {
          ...
          await cursor.delete();
          this.logger.debug('[OfflineQueue] Processed and removed operation', {
            id: op.id,
          });
        } catch (error) {
          this.logger.error(
            '[OfflineQueue] Failed to execute operation',
            error as Error,
            { id: op.id }
          );
          ...
          if (op.retryCount < 5) {
            ...
            await cursor.delete();
            this.logger.error(
              '[OfflineQueue] Max retries exceeded, dropping operation',
              undefined,
              { id: op.id }
            );
          }
