...
export async function initializeBackground(): Promise<Container> {
  ...
  const scopedTagRepository =
    container.resolve<ScopedTagRepository>('scopedTagRepository');
  ...
  const localWriteEchoTracker = container.resolve<LocalWriteEchoTracker>(
    'localWriteEchoTracker'
  );
  ...
  if (currentUser) {
    logger.info('[BOOTSTRAP] User authenticated on startup, activating Pro storage', {
