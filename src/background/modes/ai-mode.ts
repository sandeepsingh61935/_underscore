import type {
  ...
  Contradiction,
  EntityExtraction,
} from '@/content/modes/mode-interfaces';
...
export class AIMode implements IAIMode {
  ...
  async generateQuestions(highlights: HighlightData[]): Promise<string[]> {
    const request: LLMRequest = {
      systemPrompt:
        'Generate 5 comprehension questions for the given highlights. Return JSON array of strings.',
      messages: [
        { role: 'user', content: highlights.map((h) => h.text).join('\n---\n') },
      ],
      maxTokens: 512,
    };
    const result = await this.deps.provider.chat(request);
  ...
  async generateMindmap(highlights: HighlightData[]): Promise<MindmapData> {
    const request: LLMRequest = {
      systemPrompt:
        'Generate a mindmap from the highlights. Return JSON: { nodes: [{id,label}], edges: [{from,to}] }.',
      messages: [
        { role: 'user', content: highlights.map((h) => h.text).join('\n---\n') },
      ],
      maxTokens: 2048,
  ...
  async detectContradictions(highlights: HighlightData[]): Promise<Contradiction[]> {
    const request: LLMRequest = {
      systemPrompt:
        'Detect contradictions across the highlights. Return JSON array of {highlight1,highlight2,reason,confidence}.',
      messages: [
        { role: 'user', content: highlights.map((h) => h.text).join('\n---\n') },
      ],
      maxTokens: 1024,
    ...
    const result = await this.deps.provider.chat(request);
    try {
      return JSON.parse(result.text) as Contradiction[];
    } catch {
      return [];
    }
  }
  ...
  async extractEntities(highlights: HighlightData[]): Promise<EntityExtraction> {
    const request: LLMRequest = {
      systemPrompt:
        'Extract named entities from the highlights. Return JSON: { people, concepts, dates, places }.',
      messages: [
        { role: 'user', content: highlights.map((h) => h.text).join('\n---\n') },
      ],
      maxTokens: 1024,
