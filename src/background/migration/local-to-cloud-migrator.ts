  ...
  async migrate(): Promise<MigrationResult> {
    ...
    try {
      ...
      for (let i = 0; i < total; i += LocalToCloudMigrator.BATCH_SIZE) {
        ...
        const batch = localHighlights.slice(i, i + LocalToCloudMigrator.BATCH_SIZE);
        ...
        await Promise.all(
          batch.map(async (highlight) => {
            try {
              ...
              this.logger.error('Failed to migrate highlight', error as Error, {
                id: highlight.id,
              });
              result.failed++;
              consecutiveFailures++;
      ...
      if (result.failed < total) {
        // At least something succeeded or total was 0
        await this.markMigrationComplete();
      }
