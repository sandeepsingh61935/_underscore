...
(RateLimiter as unknown as { persistent: typeof RateLimiter.persistent }).persistent =
  async function persistent(
    ...
    const limiter = new RateLimiter(
      config,
      options?.logger as unknown as ILogger | undefined
    );

    if (!hasChromeStorage()) {
      // Fail closed — see ADR-019 "fail closed" decision.
      options?.logger?.warn(
        'chrome.storage.local unavailable; rate limiter is fail-closed',
        {
      ...
      (
        limiter as unknown as { tryAcquire: () => Promise<boolean> | boolean }
