...
export class TextQuoteFinder {
    ...
    private getTextBefore(range: Range): string {
        ...
        while (node && text.length < 64) { // Match prefix max length
    ...
    private getTextAfter(range: Range): string {
        ...
        while (node && text.length < 64) { // Match suffix max length
            if (node.nodeType === Node.TEXT_NODE) {
 ...
 */
export function findTextQuoteSelector(selector: TextQuoteSelector, root?: Node): Range | null {
