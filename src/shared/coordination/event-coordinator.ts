...
export class EventCoordinator {
  ...
  registerHandler<T = any>(
    ...
    const normalized: HandlerWithRollback<T> =
      typeof handler === 'function' ? { execute: handler } : handler;
  ...
  async dispatch<T = any>(eventType: string, event: T): Promise<void> {
    ...
    if (this.inTransaction) {
      throw new Error(
        'Cannot dispatch event while in transaction (prevent nested transactions)'
      );
    }
    ...
    try {
      ...
      this.logger.error(
        'Event dispatch failed, rolling back',
