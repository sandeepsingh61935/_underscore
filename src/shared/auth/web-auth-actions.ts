...
export async function verifyEmailOtp(email: string, token: string): Promise<WebAuthActionResult> {
    ...
    if (error) {
        return { success: false, error: mapAuthError('verify-email-otp', error), code: error.code };
    }
...
export async function resendEmailOtp(email: string): Promise<WebAuthActionResult> {
    ...
    if (error) {
        return { success: false, error: mapAuthError('resend-email-otp', error), code: error.code };
    }
    return { success: true };
}
...
export async function requestPasswordReset(email: string): Promise<WebAuthActionResult> {
    ...
    if (error) {
        return { success: false, error: mapAuthError('request-password-reset', error), code: error.code };
    }
    return { success: true };
}

/** Verify a password-reset code. Establishes a temporary recovery session. */
export async function verifyRecoveryOtp(email: string, token: string): Promise<WebAuthActionResult> {
    if (!EmailSchema.safeParse(email).success || !OtpSchema.safeParse(token).success) {
        return invalidInput('Enter a valid email and 6-digit code.');
    }
    ...
    if (error) {
        return { success: false, error: mapAuthError('verify-recovery-otp', error), code: error.code };
    }
    return { success: true, session: data.session };
}
...
export async function updatePassword(password: string): Promise<WebAuthActionResult> {
    ...
    if (error) {
        return { success: false, error: mapAuthError('update-password', error), code: error.code };
    }
    return { success: true };
}
