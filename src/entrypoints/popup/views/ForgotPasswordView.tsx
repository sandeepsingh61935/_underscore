 ...
 */
export function ForgotPasswordView({ onCodeSent, onBack }: ForgotPasswordViewProps): React.ReactElement {
    ...
    const handleSubmit = async (e: React.FormEvent): Promise<void> => {
        ...
        if (!result.success) {
            if (isRateLimitCode(result.code)) {
                ...
                setError(result.error || 'Unable to send a reset code right now. Please try again.');
            } else {
    ...
    return (
        <AuthScreenShell
            ...
            error={error}
            footer={
                <Button type="button" variant="ghost" onClick={onBack} style={{ alignSelf: 'center', display: 'flex', margin: '0 auto' }}>
                    Back to sign in
                </Button>
            }
        >
            ...
            <form onSubmit={(e) => void handleSubmit(e)} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <label htmlFor="popup-forgot-email" className="u-kicker" style={{ color: 'var(--ink-3)' }}>
                        Email
                    </label>
                    <Input
