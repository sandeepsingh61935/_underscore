...
const noop = (): void => {};
const handleModeSelect = (_modeId: string): void => {};
const handleCollectionClick = (_domain: string): void => {};
  ...
  render: () => (
    <div className="w-[400px] h-[600px] flex flex-col bg-surface border shadow-xl">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-outline bg-surface-container/80 backdrop-blur-md sticky top-0 z-10">
        ...
        <UserMenu user={mockUser} onLogout={noop} />
      </header>
          ...
          <div className="space-y-2">
            {mockCollections.map((collection) => (
              <CollectionCard
                ...
                onClick={() => handleCollectionClick(collection.domain)}
              />
            ))}
          </div>
        </section>
...
export const Unauthenticated: Story = {
  render: () => (
    <div className="w-[400px] h-[600px] flex flex-col bg-surface border shadow-xl">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-outline bg-surface-container/80 backdrop-blur-md sticky top-0 z-10">
        <Logo className="scale-75 origin-left" showText={true} />
        <button className="text-label-large text-primary">Sign In</button>
      </header>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-8">
        ...
        <section className="flex-1 flex flex-col items-center justify-center py-12 text-center">
          <Text variant="small" muted className="mb-4">
            Sign in to sync your highlights
          </Text>
          <button className="px-4 py-2 bg-primary text-primary-foreground rounded-full text-label-large">
            Get Started
          </button>
        </section>
      </div>
